...
## Supported Engine Releases (as of September 2026)

1. **Current Production Releases**: Chromium `140` through `151`
   - Fully compatible with modern Client Hints, Partitioned Cookies, and WebGPU standards.
   - Actively maintained and validated for commercial deployments.
2. **Legacy / Development Targets**: Chromium `130` through `135`
...
The full version (`major.minor.build.patch`) is extracted from the executable's metadata and used by `BrowserVersion` to ensure build-level coherence across DOM and network headers.
