...
from backend.ai_generator import KIMI_MODEL, build_system_prompt, generate_fingerprint_ai
        ...
        try:
            with patch("backend.ai_generator._call_via_racing_proxy", return_value=None):
                ...
                for i in range(samples_target):
                    ...
                    if h in hashes_seen:
                        print(f"[Orchestrator] Fail: Duplicate fingerprint detected (safe hash).")
                        passed_uniqueness = False
