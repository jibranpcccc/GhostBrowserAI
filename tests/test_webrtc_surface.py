    ...
    async def run_test():
        ...
        try:
            ...
            try:
                ...
                rtc_defined = await page.evaluate("typeof RTCPeerConnection !== 'undefined'")
                if rtc_defined:
                    print("[PASS] RTCPeerConnection is defined")
                else:
                    print("[FAIL] RTCPeerConnection is not defined")
                    all_passed = False

                webdriver_flag = await page.evaluate("navigator.webdriver")
                if webdriver_flag is False:
                    print("[PASS] navigator.webdriver === false")
                else:
                    print(f"[FAIL] navigator.webdriver is not false: {webdriver_flag}")
                    all_passed = False
                ...
                try:
                    create_result = await page.evaluate("""
                        (() => {
                            try {
                                ...
                                return { success: true, type: typeof pc };
                            } catch (e) {
                    ...
                    if create_result["success"]:
                        print(f"[PASS] RTCPeerConnection created successfully: type={create_result['type']}")
                    else:
                        print(f"[FAIL] RTCPeerConnection creation failed: {create_result['error']}")
                        all_passed = False
                except Exception as ex:
                    print(f"[FAIL] RTCPeerConnection creation threw: {ex}")
                    all_passed = False

                rtc_session = await page.evaluate("typeof RTCSessionDescription !== 'undefined'")
                if rtc_session:
                    print("[PASS] RTCSessionDescription exists")
                else:
                    print("[FAIL] RTCSessionDescription does not exist")
                    all_passed = False

                rtc_ice = await page.evaluate("typeof RTCIceCandidate !== 'undefined'")
                if rtc_ice:
                    print("[PASS] RTCIceCandidate exists")
                else:
                    print("[FAIL] RTCIceCandidate does not exist")
                    all_passed = False
