    ...
    async def run_test():
        ...
        try:
            ...
            try:
                ...
                sw_available = await page.evaluate("typeof navigator.serviceWorker !== 'undefined'")
                if sw_available:
                    print("[PASS] navigator.serviceWorker is available")
                else:
                    print("[FAIL] navigator.serviceWorker is not available")
                    all_passed = False

                controller = await page.evaluate("navigator.serviceWorker ? navigator.serviceWorker.controller : 'no_sw'")
                if controller is None or controller == "no_sw":
                    print(f"[PASS] navigator.serviceWorker.controller is null/absent: {controller}")
                else:
                    print(f"[FAIL] navigator.serviceWorker.controller is not null: {controller}")
                    all_passed = False

                try:
                    reg_result = await page.evaluate("""
                        (async () => {
                            try {
                                await navigator.serviceWorker.register('/sw-test.js');
                    ...
                    if not reg_result["success"]:
                        print(f"[PASS] Service worker registration blocked: {reg_result['error']}")
                    else:
                        print("[FAIL] Service worker registration succeeded (should be blocked)")
                        all_passed = False
                except Exception as ex:
                    print(f"[PASS] Service worker registration threw exception: {ex}")
