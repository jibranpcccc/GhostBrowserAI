...
# Disable the live-network CookieWarmer for the entire test suite.
# CookieWarmer requires an explicit opt-in (GHOSTBROWSER_ENABLE_COOKIE_WARMER=1)
# to visit real sites; tests must never make external network requests.
...
# Global flag so post-suite status logic knows whether coverage was omitted.
IS_FAST_MODE = False
...
async def run_tests():
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Input Validation Tests (skipped in fast mode)", False, is_skipped=True)
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Host OS Restriction Tests (skipped in fast mode)", False, is_skipped=True)
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Chromium Version Binding Tests (skipped in fast mode)", False, is_skipped=True)
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Creation Transaction and Fallbacks (skipped in fast mode)", False, is_skipped=True)
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Deletion Safety Tests (skipped in fast mode)", False, is_skipped=True)
    ...
    try:
        with patch("backend.profile_creator.generate_fingerprint_ai", mock_ai_gen):
            ...
            # Production mode must not accept this offline mock: it has no
            # trusted Kimi provenance and no verified proxy. Older tests
            # expected it to launch directly, which would now be a policy leak.
            os.environ["GHOSTBROWSER_TEST_ENV"] = "0"
            try:
                p_prod = await profile_creator.create_zero_leak_profile(name="Prod-Test-Profile")
                collector.add_check(
                    "Production mode rejects unverified proxyless fixture",
                    p_prod.get("status") == "error" and "profile" not in p_prod,
                )
            finally:
                os.environ["GHOSTBROWSER_TEST_ENV"] = "1"
            ...
            try:
                ...
                if relaunch_res["status"] == "success":
                    ...
                    worker_pre_ok = len(pre_worker_reqs) > 0 and not any(
                        key.startswith("sec-ch-ua")
                        for key in pre_worker_reqs[0]["headers"]
                    )
                    collector.add_check("Worker fetch pre Accept-CH matches native Chromium hint omission", worker_pre_ok, check_id="phase4.worker_headers_pre_accept_ch")
                    ...
                    worker_network_ua = worker_headers.get("user-agent", "")
                    worker_native_hint_omission = bool(worker_headers) and not any(
                        key.startswith("sec-ch-ua")
                        for key in worker_headers
                    )
                    collector.add_check(
                        "Worker fetch post Accept-CH matches native Chromium hint omission",
                        worker_native_hint_omission and worker_network_ua == expected_ua,
                        check_id="phase4.worker_headers_post_accept_ch",
                    )
                    ...
                    # 6. Direct profiles retain native Service Worker behavior.
                    # Proxied profiles are covered separately by the fail-closed
                    # proxy audit and remain blocked until transport is proven.
                    server1_requests.clear()
                    sw_status = await page_re.evaluate("""
                        async () => {
                            ...
                            try {
                                ...
                                result.registerResolved = true;
                            } catch (e) {
                                result.registerError = e.name;
                            }
                            ...
                            result.readyResolved = await Promise.race([
                    ...
                    """)
                    sw_script_requested = any(
                        request["path"].split("?", 1)[0] == "/sw.js"
                        for request in server1_requests
                    )
                    sw_direct_native_ok = (
                        sw_status.get("supported") is True
                        and sw_status.get("registerResolved") is True
                        and (sw_status.get("active") or sw_status.get("readyResolved"))
                        and sw_script_requested
                    )
                    collector.add_check(
                        "Direct profile uses native service-worker registration and script transport",
                        sw_direct_native_ok,
                        check_id="phase4.service_worker_policy",
                    )
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Process Isolation & Multi-Profile Tests (skipped in fast mode)", False, is_skipped=True)
    ...
    if not IS_FAST_MODE:
        ...
        collector.add_check("Sequential Lifecycle Stability Loop (skipped in fast mode)", False, is_skipped=True)

    # 8. Audio and AI-declared surface regression
    print("\n--- Running Audio and AI-Declared Surface Regression ---")
    try:
        audio_proc = await asyncio.create_subprocess_exec(
            sys.executable, "tests/test_audio_noise_extended.py",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
        audio_stdout, audio_stderr = await asyncio.wait_for(audio_proc.communicate(), timeout=300)
        collector.add_check("Audio noise extended regression passes", audio_proc.returncode == 0)

        surface_proc = await asyncio.create_subprocess_exec(
            sys.executable, "tests/test_ai_declared_surface_audit.py",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
        )
...
def run_post_suite_checks():
    ...
    probe_ok = collector.check_results.get("At least one request recorded on server 1", False)

    # 2. Secure context confirmed
    secure_ctx_ok = collector.check_results.get("window.isSecureContext is true", False)

    # 3. Executable path/version matched
    exe_ver_ok = collector.check_results.get("Executable version exact match PASS", False)
    ...
    sw_policy_ok = (collector.required_status.get("phase4.service_worker_policy") == "passed")
    ...
    matrix = [
        ...
        ("3", "Executable path/version matched", exe_ver_ok, f"Chrome {inst_maj}"),
        ("4", "Initial-request hints", init_hints_ok, "Low entropy only"),
        ("5", "Post-Accept-CH hints", post_accept_ok, "Profile-persisted via opted_origins.json"),
        ...
        ("9", "Worker headers", worker_headers_ok, "Native omission; profile UA retained"),
        ("10", "Service-worker policy", sw_policy_ok, "Native for direct profiles; blocked for proxied profiles"),
    ...
    if collector.failed > 0:
        print("\nSTATUS: NOT READY (Some checks failed)")
        sys.exit(1)
    elif IS_FAST_MODE and collector.skipped > 0:
        print("\nSTATUS: FAST PASS (coverage omitted; not release ready)")
        sys.exit(0)
    else:
        print("\nSTATUS: READY")
        sys.exit(0)
