...
class AICoherenceValidator:
    ...
    def validate(self, fingerprint: dict) -> dict:
        ...
        if not isinstance(fingerprint, dict):
            return {
                "score": 0,
                "issues": ["Fingerprint must be a key-value dictionary."],
                "passed": False
            }

        # Strict Schema Validation: fail-closed on missing essential keys
        essential_keys = [
            "userAgent", "webgl_vendor", "webgl_renderer",
            "cpu_cores", "memory_gb", "timezone", "locale",
            "screen_resolution", "sec_ch_ua", "sec_ch_ua_platform"
        ]
        for k in essential_keys:
            if k not in fingerprint or fingerprint[k] is None:
                score -= 40
                issues.append(f"Missing essential fingerprint property: '{k}'")

        # 1. OS Validation
        os_name = fingerprint.get("os", "")
        if not os_name or os_name not in ["Windows", "Mac", "Linux"]:
            score -= 80
            issues.append(f"Missing or invalid OS value: '{os_name}'. Must be Windows, Mac, or Linux.")
        ...
        if os_name == "Linux" and sec_plat != "Linux":
            score -= 50
            issues.append(f"OS is Linux but ClientHints Platform is {sec_plat}")
        ...
        if not ua_match:
            score -= 40
            issues.append("User-Agent is missing Chrome version.")
        if not sec_match:
            score -= 40
            issues.append("Client Hint (sec_ch_ua) is missing Chromium version.")
        ...
        # 4b. client_hints.uaFullVersion must match the same major version as UA/sec-ch-ua
        client_hints = fingerprint.get("client_hints") or {}
        ua_full = client_hints.get("uaFullVersion", "")
        if ua_full:
            ua_full_match = re.search(r'^(\d+)', str(ua_full))
            if ua_full_match:
                expected_major = ua_match.group(1) if ua_match else (sec_match.group(1) if sec_match else None)
                if expected_major and ua_full_match.group(1) != expected_major:
                    score -= 60
                    issues.append(f"client_hints.uaFullVersion major mismatch! Expected {expected_major}, got {ua_full_match.group(1)}")
        ...
        if "nvidia" in vendor and "geforce" not in renderer and "rtx" not in renderer and "gtx" not in renderer and "quadro" not in renderer and "titan" not in renderer and "nvidia a" not in renderer and "a10" not in renderer:
            score -= 30
